...
`.claude/agent-memory/` 以下のファイルはエージェントの知見蓄積用であり、**絶対に削除しないこと**。コミット時にはこれらのファイルも含めること。

## スキル定義の同期

`.agents/skills/` と `.claude/skills/` に同名スキルが存在する場合、片方だけを修正してはならない。スキルを修正するときは対応する両方の `SKILL.md` を同じ意図で同時に更新し、差異を残さないこと。

...
async function installPageAsync(page: Page, time: string): Promise<void> {
    const leftRows = [
    ...
    fs['.masterdataeditor/settings.json'] = JSON.stringify({exportValidationDateTime: time, exportBeginDateColumnName: 'available_from', exportEndDateColumnName: 'available_until'});
    for (const file of FILES) {
...
test('時刻が未設定なら設定案内を表示し、通常比較には切り替えられる', async ({page}) => {
...
test('設定時刻の変更で一覧を再計算し、開いた比較タブは元の時刻を維持する', async ({page}) => {
    await installPageAsync(page, TIME);
    await compareAsync(page, true);
    await expect(names(page)).toHaveText(['active', 'deleted', 'plain']);
    await page.locator('.branch-compare-file-item').filter({hasText: 'active'}).click();
    ...
    await input.fill('2027-06-01T12:00:00');
    await input.blur();

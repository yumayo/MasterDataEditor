...
//   バリデーションがスキップされる。そのため cell-error が付与されない。
//
// テストが RED になる理由:
...
//
// テストケース一覧:
//   1. 複合主キーの重複検出（両方のPK構成列のセルに cell-error が付与される）
//   2. 複合主キーの重複解消（cell-error が除去される）
...
test.describe('テストケース1: 複合主キーの重複検出', () => {
    ...
    test(
        '3行目のshop_idを1に変更すると(1,101)が重複し、1行目と3行目の両PK構成列に cell-error が付与される',
        async ({ page }) => {
            const table = await openTableAsync(page, 'shop_product');

            // 初期状態: 重複なし。全PK構成列に cell-error クラスがないことを確認する
            ...
            await expect(row1ShopId).not.toHaveClass(/cell-error/);
            await expect(row1ProductId).not.toHaveClass(/cell-error/);
            await expect(row2ShopId).not.toHaveClass(/cell-error/);
            await expect(row2ProductId).not.toHaveClass(/cell-error/);
            await expect(row3ShopId).not.toHaveClass(/cell-error/);
            await expect(row3ProductId).not.toHaveClass(/cell-error/);

            // 3行目の shop_id（=2）を 1 に変更して (1,101) の重複を発生させる
            await editCellAsync(table, page, 2, 0, '1');

            // 1行目と3行目の shop_id・product_id 両セルに cell-error が付与されることを確認する
            // 現行実装は config.primaryKeyColumnName（="id"）の単一PKしか検出しないため RED
            await expect(row1ShopId).toHaveClass(/cell-error/);
            await expect(row1ProductId).toHaveClass(/cell-error/);
            await expect(row3ShopId).toHaveClass(/cell-error/);
            await expect(row3ProductId).toHaveClass(/cell-error/);

            // 2行目は (1,102) で重複していないため cell-error が付与されないことを確認する
...
test.describe('テストケース2: 複合主キーの重複解消', () => {
    ...
    test(
        '重複状態から shop_id を別の値に変更すると cell-error が除去される',
        async ({ page }) => {
            const table = await openTableAsync(page, 'shop_product');
            ...
            // cell-error クラスがすべてのセルから除去されることを確認する
...
test.describe('テストケース3: 複合主キーで空値は重複チェック対象外', () => {
    ...
    test(
        '複合PKの構成列のいずれかが空文字の場合、その行は重複チェック対象外となり cell-error が付与されない',
        async ({ page }) => {
            const table = await openTableAsync(page, 'shop_product');
            ...
            // 空値を持つ行には cell-error が付与されないことを確認する
...
test.describe('テストケース4: 初期表示で複合主キー重複が検出される', () => {
    ...
    test(
        'テーブルを開いた時点で複合PK重複行の両PK構成列に cell-error が付与される',
        async ({ page }) => {
            const table = await openTableAsync(page, 'shop_product');
            ...
            // テーブルオープン直後に複合PK重複の両行・両列に cell-error が付与されることを確認する
            // 現行実装は config.primaryKeyColumnName（="id"）列が存在しないテーブルでは
            // validatePkDuplicates() が早期リターンするため RED
            await expect(row1ShopId).toHaveClass(/cell-error/);
            await expect(row1ProductId).toHaveClass(/cell-error/);
            await expect(row2ShopId).toHaveClass(/cell-error/);
            await expect(row2ProductId).toHaveClass(/cell-error/);

            // 3行目は (2,101) で重複していないため cell-error が付与されないことを確認する
            await expect(row3ShopId).not.toHaveClass(/cell-error/);
            await expect(row3ProductId).not.toHaveClass(/cell-error/);
        },
    );
...
test.describe('テストケース5: 単一主キーとの後方互換性', () => {
    ...
    test(
        ...
        async ({ page }) => {
            ...
            await expect(row1PkCell).not.toHaveClass(/cell-error/);
            await expect(row2PkCell).not.toHaveClass(/cell-error/);

            // 2行目の id を 1 に変更して単一PK重複を発生させる
            await editCellAsync(table, page, 1, 0, '1');

            // 両方の id セルに cell-error が付与されることを確認する
            // 単一PK対応は現行実装で既に動作しているため GREEN
            await expect(row1PkCell).toHaveClass(/cell-error/);
            await expect(row2PkCell).toHaveClass(/cell-error/);

            // 3行目（id=3）には cell-error が付与されないことを確認する
            const row3PkCell = getDataCell(table, 2, 0);
            await expect(row3PkCell).not.toHaveClass(/cell-error/);
        },
    );
});

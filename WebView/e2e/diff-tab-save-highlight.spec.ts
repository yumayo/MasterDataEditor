...
// =============================================================================
// サブディレクトリ環境での差分タブ保存後git差分ハイライト更新を検証する
//
// 背景:
//   workDir が git リポジトリのサブディレクトリ（例: sample-workdir/）の場合、
//   C#側の GetDataPrefix が "sample-workdir/data/" を返し、
//   git status のパスは "subdir/data/quest_reward.csv" 形式になる。
//   差分タブを開くときは source-control-panel.ts が entry.path をそのまま
//   gitShowAsync() に渡すため正しく動作する。
//   保存後の refreshGitDiffForDiffTabAsync() でも gitPath（"subdir/data/quest_reward.csv"）を
//   そのまま gitShowAsync に渡すため、サブディレクトリ環境でも正しくHEAD版CSVを取得できる。
//
// テスト設計:
//   __mockGitHeadFiles に "subdir/data/quest_reward.csv" でのみHEAD版CSVを登録し、
//   "data/quest_reward.csv" キーは登録しない。
//   gitPath がそのまま渡されることで正しいキーでHEAD版CSVが取得され、
//   変更セルのみに cell-git-changed が付与されることを検証する。
// =============================================================================

// サブディレクトリ環境: git status が返すパスにサブディレクトリプレフィックスが付く
const SUBDIR_GIT_STATUS = {
    changes: [{ path: "subdir/data/quest_reward.csv", tableName: "quest_reward", isNew: false }],
    staged: [] as { path: string; tableName: string; isNew: boolean }[],
};

// サブディレクトリ環境: HEAD版ファイルマップ（キーはgit status の path と同一）
// "data/quest_reward.csv" キーは登録しない（gitPathがそのまま使われることの検証）
const SUBDIR_HEAD_FILES: Record<string, string> = {
    "subdir/data/quest_reward.csv": HEAD_QUEST_REWARD_CSV,
};

// フィクスチャ型定義
interface SubdirDiffTabSaveHighlightFixtures {
    /** サブディレクトリ環境のgit差分状態をセットアップした状態でページを開く */
    subdirDiffTabSaveHighlightPage: void;
}

/**
 * サブディレクトリ環境の差分タブ保存後ハイライト検証テスト用フィクスチャ
 * addInitScript は goto より前に実行する必要があるため、
 * installMockApiAsync より前に __mockGitStatus / __mockGitHeadFiles を設定する
 */
const testSubdir = base.extend<SubdirDiffTabSaveHighlightFixtures>({
    subdirDiffTabSaveHighlightPage: async ({ page }, use) => {
        await page.addInitScript((args: {
            status: { changes: { path: string; tableName: string; isNew: boolean }[]; staged: { path: string; tableName: string; isNew: boolean }[] };
            headFiles: Record<string, string>;
        }) => {
            (window as unknown as { __mockGitStatus: object }).__mockGitStatus = args.status;
            (window as unknown as { __mockGitHeadFiles: Record<string, string> }).__mockGitHeadFiles = args.headFiles;
        }, { status: SUBDIR_GIT_STATUS, headFiles: SUBDIR_HEAD_FILES });
...
testSubdir.describe('サブディレクトリ環境での差分タブ保存後git差分ハイライト更新', () => {

    // -------------------------------------------------------------------------
    // テスト: workDirがサブディレクトリの場合、差分タブ右ペインで編集・保存後に
    //         変更セルのみcell-git-changedが付き、未変更セルには付かないこと
    //
    // 検証手順:
    //   1. ソースコントロールパネルを開く
    //   2. quest_reward の差分タブを開く（entry.path="subdir/data/..."を使うため正常に開く）
    //   3. 右ペインのセル（id=2, name列）を編集する
    //   4. Ctrl+S で保存する
    //   5. 編集したセルに .cell-git-changed が付与されていることを確認する
    //   6. 変更していないセルには .cell-git-changed が付いていないことを確認する
    //      gitPath（"subdir/data/quest_reward.csv"）がそのまま gitShowAsync に渡されるため、
    //      サブディレクトリ環境でも正しくHEAD版CSVを取得でき、変更セルのみにcell-git-changedが付与される
    // -------------------------------------------------------------------------
    testSubdir(
        'サブディレクトリ環境で差分タブ保存後に変更セルのみcell-git-changedが付くこと',
        async ({ page, subdirDiffTabSaveHighlightPage: _subdirDiffTabSaveHighlightPage }) => {
            // ソースコントロールパネルを開く
            await page.locator('[data-panel="sourceControl"]').click();
            ...
            // HEAD版と同じ値のセルには .cell-git-changed が付いていないことを確認する
            // gitPath（"subdir/data/quest_reward.csv"）がそのまま gitShowAsync に渡されるため、
            // サブディレクトリ環境でも正しくHEAD版CSVを取得でき、変更セルのみにcell-git-changedが付与される
            const unchangedIdCell = getDataCell(rightTable, 1, 0);
            await expect(unchangedIdCell).not.toHaveClass(/cell-git-changed/);

            // id=3 の全セルも変更なし → cell-git-changed が付いていないことを確認する
            const unchangedRow2Id = getDataCell(rightTable, 2, 0);
            const unchangedRow2Name = getDataCell(rightTable, 2, 1);
            await expect(unchangedRow2Id).not.toHaveClass(/cell-git-changed/);
            await expect(unchangedRow2Name).not.toHaveClass(/cell-git-changed/);
        },
    );

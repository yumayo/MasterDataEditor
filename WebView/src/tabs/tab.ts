...
import type {UiStateStore, UiTabsState, UiStoredTab, UiStoredDiffTab, UiStoredBranchCompareDiffTab, UiStoredBranchCompareListTab, UiScrollPosition, UiStoredEditorTableState, UiStoredSelectionState, UiStoredFormPanelState} from "../app/ui-state";
...
export class Tab {
    ...
    private readonly diffTabs: Map<string, DiffTab | BranchCompareListTab>;
    ...
    /** 差分タブ読み込みインジケーターの競合防止トークン */
    private readonly diffLoadingTokens: Map<string, number>;
    ...
    switchToExistingTab(name: string): void {
        // 特殊タブは tabStates に登録されないため、tabButtons で存在を確認する
    ...
    private setActiveTabName(name: string | false): void {
        ...
        if (changed) {
            this.notifyActiveTabChangeListener();
            this.notifyBranchCompareSelection();
        }
    }
    ...
    private connectDiffTabUiState(diffTabName: string, diffTab: DiffTab | BranchCompareListTab): void {
    ...
    private async openRestoredDiffTabAsync(diffTabName: string): Promise<boolean> {
        ...
        try {
            if (metadata.kind === 'branchCompareList') {
                await this.loadBranchCompareListTabAsync(diffTabName, metadata, loadingToken, null);
                return true;
            }
            ...
            if (entry.isNew) {
                ...
                await this.openDiffTabAsync(tableName, isStaged, schemaJson, headerOnlyCsv, currentCsv, entry.path, null, null, entry.isNew, loadingToken);
                return true;
            }
            ...
            await this.openDiffTabAsync(tableName, isStaged, schemaJson, headCsv, currentCsv, entry.path, null, null, entry.isNew, loadingToken);
            return true;
        } catch (error: unknown) {
            this.finishDiffTabLoading(diffTabName, loadingToken, true);
    ...
    private performCloseTab(name: string): void {
        ...
        if (name.startsWith(DIFF_TAB_PREFIX)) {
            ...
            this.diffLoadingTokens.delete(name);
        }
    ...
    private activateTabButton(name: string): TabState | null {
        ...
        if (name.startsWith(DIFF_TAB_PREFIX)) {
            if (this.loadingDiffTabNames.has(name)) {
                this.activateDiffLoadingTab(name);
                return null;
            }
        ...
        // 設定タブ・テーブル定義タブ・API詳細タブ・Viewプラグインタブから通常テーブルタブへの復帰時: rightSlot・ナビゲーションバーを復元する
    ...
    private activateDebugApiDetailTab(): void {
        // 設定タブ・差分タブ・テーブル定義タブ・Viewプラグインタブがアクティブだった場合: leaveSettingsMode() で rightSlot を復元しておく
    ...
    /** 同じ固定コミット・ファイル・出力条件の一覧は既存タブを再選択する。 */
    async openBranchCompareListTabAsync(metadata: UiStoredBranchCompareListTab, signal: AbortSignal): Promise<void> {
        const identity = (value: UiStoredBranchCompareListTab): string => JSON.stringify([
            value.leftCommit, value.rightCommit, value.leftLabel, value.rightLabel,
            value.files.map(file => [file.path, file.tableName, file.status]),
            value.exportFilter ? [value.exportFilter.leftDateTime, value.exportFilter.rightDateTime, value.exportFilter.beginColumnName, value.exportFilter.endColumnName] : null,
        ]);
        const serialized = identity(metadata);
        const existing = [...this.diffTabMetadata].find(([, stored]) => stored.kind === 'branchCompareList' && identity(stored) === serialized);
        if (existing) {
            this.enableTabButton(existing[0]);
            return;
        }
        const baseName = DIFF_TAB_PREFIX + '一覧 (' + metadata.leftLabel.slice(0, 32) + ' ↔ ' + metadata.rightLabel.slice(0, 32) + ') [' + metadata.leftCommit.slice(0, 7) + ' ↔ ' + metadata.rightCommit.slice(0, 7) + ']';
        let name = baseName;
        let suffix = 2;
        while (this.tabButtons.some(button => button.name === name)) name = baseName + ' (' + suffix++ + ')';
        const token = this.beginDiffTabLoadingForName(name, '差分一覧を読み込み中…', false);
        if (token === false) return;
        this.diffTabMetadata.set(name, metadata);
        await this.loadBranchCompareListTabAsync(name, metadata, token, signal);
    }

    private async loadBranchCompareListTabAsync(name: string, metadata: UiStoredBranchCompareListTab, token: number, signal: AbortSignal | null): Promise<void> {
        const cancelled = (): boolean => signal?.aborted === true || this.diffLoadingTokens.get(name) !== token;
        const cancelLoading = (): void => {
            if (this.diffLoadingTokens.get(name) !== token) return;
            this.diffTabMetadata.delete(name);
            this.finishDiffTabLoading(name, token, true);
        };
        // 中断を即時に反映し、次の一覧操作が中断済みの読み込みタブを再利用しないようにする。
        signal?.addEventListener('abort', cancelLoading, {once: true});
        const sections: BranchCompareListSection[] = [];
        try {
            // 元CSVは1テーブルずつ取得し、workerからは前後5行に圧縮した結果だけを受け取る。
            ...
            this.finishDiffTabLoading(name, token, false);
            if (wasActive) this.enableTabButton(name);
    ...
    private async saveActiveTableAsync(): Promise<void> {
        ...
        if (this.activeTabName.startsWith(DIFF_TAB_PREFIX)) {
            const diffTab = this.diffTabs.get(this.activeTabName);
            if (diffTab instanceof DiffTab) diffTab.saveRightPane();
            return;
        }
        // 通常テーブルの場合
        const state = this.tabStates.get(this.activeTabName);
    ...
    private async createOrReplaceDiffTabAsync(
        diffTabName: string,
        tableName: string,
        isStaged: boolean,
        schemaJson: string,
        ...
        if (tabButton === undefined) {
            this.finishDiffTabLoading(diffTabName, token, true);
            return;
        }
    ...
    private activateDiffTab(diffTabName: string): void {
        ...
        // 設定タブ・テーブル定義タブ・API詳細タブ・Viewプラグインタブがアクティブだった場合: leaveSettingsMode() で rightSlot を復元しておく
    ...
    private createLoadingWrapper(name: string, labelText: string = `読み込み中: ${name}`): HTMLElement {
    ...
    beginDiffTabLoading(tableName: string): number | false {
        const diffTabName = DIFF_TAB_PREFIX + tableName;
        return this.beginDiffTabLoadingForName(diffTabName, `差分読み込み中: ${tableName}`, true);
    }

    cancelDiffTabLoading(tableName: string, token: number): void {
        const diffTabName = DIFF_TAB_PREFIX + tableName;
        const hadExistingDiffTab = this.diffTabs.has(diffTabName);
        const finished = this.finishDiffTabLoading(diffTabName, token, !hadExistingDiffTab);
        if (finished && hadExistingDiffTab) {
            this.enableTabButton(diffTabName);
        }
    }
    ...
    private beginDiffTabLoadingForName(diffTabName: string, labelText: string, respectDirtyState: boolean, preview: boolean = false): number | false {
        ...
        const token = ++this.nextDiffLoadingToken;
        this.diffLoadingTokens.set(diffTabName, token);
        this.loadingDiffTabNames.add(diffTabName);

        let wrapperElement = this.loadingWrapperElements.get(diffTabName);
        if (wrapperElement === undefined) {
            wrapperElement = this.createLoadingWrapper(diffTabName, labelText);
            this.loadingWrapperElements.set(diffTabName, wrapperElement);
            this.editor.appendChild(wrapperElement);
        } else {
            const label = wrapperElement.querySelector('.editor-table-loading-label');
    ...
    private finishDiffTabLoading(diffTabName: string, token: number, removeTabIfEmpty: boolean): boolean {
        if (this.diffLoadingTokens.get(diffTabName) !== token) return false;
        this.diffLoadingTokens.delete(diffTabName);
        this.loadingDiffTabNames.delete(diffTabName);
        this.removeLoadingWrapper(diffTabName);
    ...
    private activateDiffLoadingTab(diffTabName: string): void {
        if (this.activeTabName && !this.isFullWidthSpecialTabName(this.activeTabName)) {
            const previousState = this.tabStates.get(this.activeTabName);
            if (previousState) {
                this.deactivateTabState(previousState);

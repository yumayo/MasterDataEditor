...
export class EditorTable {
    ...
    private updateSelectionFromPoint(clientX: number, clientY: number): void {
        ...
        const clampedX = this.clamp(clientX, viewport.left, viewport.right - 1);
        const clampedY = this.clamp(clientY, viewport.top, viewport.bottom - 1);
        ...
        for (const element of elements) {
            ...
            const position = EditorTable.getCellPosition(element, this.element);
            if (!position) return;
    ...
    private getHeaderHeight(): number {
        if (this.element.children.length === 0) return 0;

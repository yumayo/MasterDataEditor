...
export declare const VIEW_PLUGIN_API_VERSION: 2;
...
export interface EditorDataAPI {
    ...
    readTableDataAsync(tableName: string): Promise<{
        header: string[];
        rows: string[][];
    } | null>;
...
export interface EditorEditAPI {
    ...
    setCellValues(tableName: string, changes: Array<{
        row: number;
        column: number;
        value: string;
    }>): boolean;
    setCellValueAsync(tableName: string, row: number, column: number, value: string): Promise<boolean>;
    setCellValuesAsync(tableName: string, changes: Array<{
        row: number;
        column: number;
        value: string;
    }>): Promise<boolean>;
...
export interface EditorEventsAPI {
    onTableOpened(handler: (event: {
        tableName: string;
    }) => void): EditorDisposable;
    onTableClosed(handler: (event: {
        tableName: string;
    }) => void): EditorDisposable;
    onCellChanged(handler: (event: EditorCellChangeEvent) => void): EditorDisposable;
    onTableSaved(handler: (event: {
        tableName: string;
    }) => void): EditorDisposable;
    onRowSelected(handler: (event: {
        tableName: string;
        rowIndex: number;
    }) => void): EditorDisposable;
}

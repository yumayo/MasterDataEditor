...
テストが RED の場合、アサーションエラーだけでなく `.CONTEXT/dump/` 配下のダンプファイルを必ず確認すること。
...
ソースコードに `console.log` でデバッグログを残してよい。プロダクションビルドでは自動除去される。テスト時は `.console.log` ファイルに記録される。
...
You have a persistent Persistent Agent Memory directory at `.claude/agent-memory/typescript-tdd-developer/`. Its contents persist across conversations.

...
- **メンバ変数のnull禁止**: メンバ変数に `null` を持たせて「後から代入する」コードがないか（生焼けオブジェクト）。戻り値やローカル変数で「値が存在しない」を表す `null` の使用は許可する。
...
You have a persistent Persistent Agent Memory directory at `.claude/agent-memory/adversarial-code-reviewer/`. Its contents persist across conversations.

...
テスト失敗時、autoDumpフィクスチャがブラウザ側の未キャッチ例外と `console.error` を `.CONTEXT/dump/{specファイル名}/{テストタイトル}.errors.log` に出力する。テストが RED の場合、対応する `.errors.log` が存在するか確認し、存在すれば内容を報告に含めること。

...
全テストが `fixtures/test.ts` の `autoDump` フィクスチャにより、テスト完了後にスクリーンショットとDOMダンプを自動保存します。

...
マスターデータエディタのUXを、実際のDOM構造を読み取って評価し、具体的で実行可能なフィードバックを提供すること。
...
以下の観点でDOM構造を精査し、UXの問題点を洗い出してください:
...
#### E. DOM構造の品質
- アクセシビリティ属性（aria-*、role等）は適切か
- セマンティックなHTML要素が使われているか
- 不要なネスト、冗長な構造はないか
- CSS classの命名から意図が読み取れるか

#### F. bug-report.md との照合
...
- 抽象的な指摘は禁止。必ず具体的なDOM要素・クラス名・属性を引用すること
- 「〜すべき」だけでなく「なぜそうすべきか」をプランナーの作業シナリオで説明すること
- 良い点も必ず挙げること。改善点だけのレビューは士気を下げる
- DOM構造から読み取れる範囲でレビューすること。推測に基づく指摘は「推測」と明記すること

...
BLAME 列の枠と幅は取得前に確保し、未取得の行には `…` を表示する。
最初は現在の表示範囲の中央を含むチャンクを取得し、その結果でセル内容だけを更新する。
...
タブ切り替えでは表示状態をタブごとに保持し、戻ったときに現在の表示範囲から再取得する（初回読み込み中の切り替えも対象）。
「変更履歴を非表示」にしたタブは、戻っても非表示のままとなる。
...
| `update_blame_cells` | 初回取得・後続チャンクの結果で、現在描画中の該当BLAMEセルを更新 |

...
## 症状
bookmarks.json等のユーザー設定ファイルがdata/ディレクトリに混在しており、CSVマスターデータとの区別がつきにくい。また、WriteFileRequestがサブディレクトリ付きパスを指定した場合に親ディレクトリを自動作成しないため、userdata/ディレクトリが存在しない初回起動時にファイル書き込みが失敗する。

## 要件
- WriteFileRequestで書き込み先の親ディレクトリが存在しない場合に自動作成する
- 既存のユーザー設定ファイル（bookmarks.json）の保存先をdata/からuserdata/に移動する
...
3. フロントエンド側でbookmarks.jsonのパスをuserdata/配下に変更する
